...
export const SMS = {
  ...
  // Heads-up sent ~30 min out by the reminder cron, to confirmed bookings
  // only. Anchored to the slot time, not "in X minutes" — the trigger
  // schedule has jitter, the appointment time doesn't.
  reminderCustomer: (name: string, service: string, time: string, address?: string) =>
    `Hi ${name}, reminder: your appt. is at ${time} — SRT Cuts ${service}.${address ? ` Address: ${address}.` : ""} See you soon! — SRT`,

  reminderAdmin: (name: string, phone: string, service: string, time: string) =>
    `Up next at ${time}: ${name} (${phone}) — ${service}.`,
